...
let test_selector_is_matching s p =
  try 
    Selector.is_matching (Path.of_string p) (Selector.of_string s |> Apero.Option.get) 
  with 
    | YException e -> print_endline @@ show_yerror e; false
    | _ -> print_endline "EXCEPTION !!"; false

let test_selector s path query fragment =
  try
    let sel = Selector.of_string s |> Apero.Option.get in
...
let test_2_wildcard_selectors () =
  ...
  check_if true  __LINE__ @@ test_selector_is_matching "//a/**/c/*/e/*" "//a/b/b/b/c/d/d/c/d/e/f";
  check_if false __LINE__ @@ test_selector_is_matching "//a/**/c/*/e/*" "//a/b/b/b/c/d/d/c/d/d/e/f";
  List.iter (fun c -> let arg = Printf.sprintf "//%c" c in check_if true __LINE__ ~arg @@ test_selector_is_matching "//**" arg) acceptable_chars;
  ()
