...
# terms of the ... which is available at
#
...
class Message(object):
    ...
    def __init__(self, raw_message=None):
        ...
        self.message_code = 0x0 #16bit
        self.flags = 0x0 #16bit comprises A,S,P
        self.flag_a = 0x0 #1bit
        self.flag_s = 0x0 #1bit
        self.flag_p = 0x0 #1bit
        self.corr_id = 0x0 #64bit
        self.length = 0x0 #64bit
        self.properties = [] #key-value tuple list (string,string)
        self.data = b''

    def pack(self):
        sub_header = struct.pack('<HHII', self.message_code, self.flags, self.corr_id, self.length)
        header = sub_header
        if self.flag_p:
    ...
    def unpack(self):
        ...
        if self.flag_p:
            ...
            for i in range(0, num_p):
                ...
                k = struct.unpack("<{}s".format(key_length), key_raw)[0].decode()[:-1]
                ...
                v = struct.unpack("<{}s".format(value_length), value_raw)[0].decode()[:-1]
    ...
    def pprint(self):
        ...
        print('# FLAGS: A:{} S:{} P:{}'.format(self.flag_a,self.flag_s,self.flag_p))
        if self.flag_p:
            print('# HAS PROPERTIES'.format(self.flag_a, self.flag_s, self.flag_p))
            print('# NUMBER OF PROPERTIES: {}'.format(len(self.properties)))
            for p in self.properties:
                print('#========\n# KEY:{} VALUE: {}'.format(p.get('key'),p.get('value')))
            print('#========')
...
class OpenMessage(Message):
    def __init__(self, username, password):
        super(OpenMessage, self).__init__()
...
class CreateMessage(Message):
    def __init__(self, type, corr_id, path, id=None, cache_size=None, config=None, complete=None):
