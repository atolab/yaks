...
public class Camera
{
   ...
   {
      // Instantiate a Yaks Java API using a socket to connect the Yaks instance on 128.10.0.1:8448
      this.yaks = Yaks.getInstance("is.yaks.YaksSocketImpl", Camera.class.getClassLoader(), "host=128.10.0.1", "port=8448");
      
      this.access = yaks.createAccess(Path.ofString("//com.huawei/shenzen/videoapps/camera/"+id), 1024, Encoding.JSON);
   }
