...
import is.yaks.Yaks;
import is.yaks.websocket.StorageImpl;
import is.yaks.websocket.AccessImpl;
...
public class YaksImpl implements Yaks {
    ...
    public Access getAccess(String id) {
        ...
        switch (response.getStatus()) {
        case HttpURLConnection.HTTP_OK:
            AccessImpl access = config.getGson().fromJson(data, gsonTypes.getTypeToken(is.yaks.websocket.AccessImpl.class));
    ...
    public Storage getStorage(String id) {
        ...
        case HttpURLConnection.HTTP_OK:
            StorageImpl storage = config.getGson().fromJson(data,
                    gsonTypes.getTypeToken(is.yaks.websocket.StorageImpl.class));
            if (storage == null) {
