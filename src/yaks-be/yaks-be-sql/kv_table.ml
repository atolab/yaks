    ...
    >|= List.map (fun row -> match row with
      | k::v::e::t::[] ->
        ...
        if Selector.is_matching_path (Path.of_string k) sub_sel then
          ...
          None
      | _ -> let _ = Logs_lwt.warn (fun m -> m "[SQL]: get in KV table %s returned non k+v+e+t value: %s" storage_info.table_name (String.concat "," row)) in None
      )
...
let get_key_status ?(with_value=false) storage_info key =
  ...
  | None ->
    ...
    | (t::[])::_ -> (match Timestamp.of_string t with
      ...
      | None -> let _ = Logs_lwt.warn (fun m -> m "[SQL]: In KV table %s for key %s: failed to read timestamp: '%s'" storage_info.table_name key t) in Lwt.return Unkown)
    ...
    | x::_ -> let%lwt _ = Logs_lwt.warn (fun m -> m "[SQL]: In KV table %s for key %s: returned non k+v+e+t value: %s" storage_info.table_name key (String.concat "," x)) in Lwt.return Unkown
...
let put storage_info (path:Path.t) (tv:TimedValue.t) =
  let%lwt _ = Logs_lwt.debug (fun m -> m "[SQL]: put(%s) into kv table %s" (Path.to_string path) (storage_info.table_name)) in
  ...
  | Present_time t | Present_value {time=t; value=_} ->
    if Timestamp.Infix.(t < tv.time) then
      ...
      let _ = Logs_lwt.debug (fun m -> m "[SQL] put on %s dropped: out-of-date" (Path.to_string path)) in
      Lwt.return_unit
  | Removed (t, cleanup) ->
    if Timestamp.Infix.(t < tv.time) then
...
let update storage_info path (delta:TimedValue.t) =
  let%lwt _ = Logs_lwt.debug (fun m -> m "[SQL]: update(%s) into kv table %s" (Path.to_string path) (storage_info.table_name)) in
  let key = sql_key_from_path storage_info path in
  ...
  | Present_value tv ->
    ...
    then 
      ...
      | Error e -> let _ = Logs_lwt.warn (fun m -> m "[SQL] update on %s failed: %s" (Path.to_string path) (show_yerror e)) in Lwt.return_unit
      )
    else 
      let _ = Logs_lwt.debug (fun m -> m "[SQL] update on %s dropped: out-of-date" (Path.to_string path)) in 
      Lwt.return_unit
  | Removed (t, cleanup) ->
    if Timestamp.Infix.(t < delta.time) then
      ...
      Lwt.return_unit
  | Present_time _ -> 
    let _ = Logs_lwt.warn (fun m -> m "[SQL] update on %s failed: internal error !! Present_value was expected instead of Present_time" (Path.to_string path)) in
    Lwt.return_unit


let remove storage_info path time =
  let open Lwt.Infix in
  let%lwt _ = Logs_lwt.debug (fun m -> m "[SQL]: remove(%s) from kv table %s" (Path.to_string path) (storage_info.table_name)) in
  let key = sql_key_from_path storage_info path in
  match%lwt get_key_status storage_info key with
  | Removed _ ->
    (* do nothing *)
  ...
  | Present_time t | Present_value {time=t; value=_} ->
    if Timestamp.Infix.(t < time) then
      ...
      let _ = Logs_lwt.debug (fun m -> m "[SQL] remove on %s dropped: out-of-date" (Path.to_string path)) in
      Lwt.return_unit
  | Unkown ->
