...
module Processor = struct 
  ...
  module Make (Engine : Yaks_engine.SEngine.S ) = struct 
    ...
    let process_delete_access engine msg = 
      ...
      let ps = msg.header.properties in 
      let aid = (Properties.get Property.Access.Key.id ps) in    
    ...
    let process_delete_storage engine msg = 
      ...
      let ps = msg.header.properties in 
      let sid = (Properties.get Property.Storage.Key.id ps) in    
    ...
    let process_delete_tuple engine msg path = 
      let%lwt _ = Logs_lwt.debug (fun m -> m "FES: processing DELETE:Tuple") in
      let ps = msg.header.properties in 
      let aid = (Properties.get Property.Access.Key.id ps) in    
      match Option.bind aid Access.Id.of_string   with 
      | Some access_id -> 
        YEngine.remove engine access_id path 
        >>= fun () -> Lwt.return @@ reply_with_ok msg Properties.empty 
      | None -> Lwt.return @@ reply_with_error msg BAD_REQUEST 
    ...
    let process_delete engine msg = 
      ...
      | _ -> 
        (match get_path_payload msg with 
         | Some path -> process_delete_tuple engine msg path 
         | None -> Lwt.return @@  reply_with_error msg BAD_REQUEST)

    let process_put engine msg = 
      let%lwt _ = Logs_lwt.debug (fun m -> m "FES: processing PUT") in 
      let ps = msg.header.properties in       
      let params = Option.bind (get_path_value_list_payload msg) 
        @@ fun pvs ->
        let id = Option.bind (Properties.get Property.Access.Key.id ps) Access.Id.of_string in 
        Option.bind id 
        @@ fun aid -> Some (pvs, aid) 
      in match params with 
      | Some (pvs, aid) ->
        Lwt.join @@ List.map (fun (p,v) -> YEngine.put engine aid p v) pvs   
        >>= fun () -> Lwt.return @@ reply_with_ok msg Properties.empty
      | None -> Lwt.return @@ reply_with_error msg BAD_REQUEST
    ...
    let process_sub engine msg pusher  = 
      ...
      | Some (aid, s) ->
        ...
        let props = Properties.singleton Property.Access.Key.subscription_id (SubscriberId.to_string subid) in 
        Lwt.return @@ reply_with_ok msg props         
      | None -> Lwt.return @@ reply_with_error msg BAD_REQUEST
    ...
    let process_unsub engine msg = 
      ...
      let params = 
        ...
        let oaid = (Properties.decode_property_value (Option.get <.> Access.Id.of_string) Property.Access.Key.id ps) in
        oaid >>= fun aid -> 
